...
"""
Dash web version of the PyQt6 Timesheet app.
...
Run:
    pip install dash dash-bootstrap-components openpyxl requests
...
def generate_task_row_component(row_idx: int, row_data: Dict[str, Any]) -> html.Div:
    ...
    days = row_data.get(
        "days",
        [{"sessions": [], "notes": "", "running_start": None} for _ in range(7)],
    )
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.Div(
                                format_hours_hhmm(hrs),
                                id={'type': 'hours-display', 'index': f"{row_idx}-{di}"},
                                style={
                                    "fontWeight": "bold",
                                    "fontSize": "14px",
                                    "textAlign": "center",
                            ...
                            dbc.Button(
                                ...
                                id={"type": "toggle-btn", "index": f"{row_idx}-{di}"},
                                color="danger" if running else "success",
                                size="sm",
                                style={"width": "100%", "marginTop": "6px"},
                            ),
                            dcc.Textarea(
                                value=days[di].get("notes", ""),
                                id={"type": "notes-input", "index": f"{row_idx}-{di}"},
                                style={
                                    "width": "100%",
                                    "height": "70px",
                                    "marginTop": "6px",
            ...
            dbc.Row(
                [
                    dbc.Col(
                        html.Div(
                            [
                                html.B(task or "(No Task)"),
                                html.Div(
                                    subtask or "(No Subtask)",
                    ...
                    *day_cells,
                    ...
                    dbc.Col(
                        dbc.Button(
                            "🗑 Delete",
                            id={"type": "delete-row", "index": str(row_idx)},
                            color="danger",
                            size="sm",
                        ),
                        width="auto",
...
app.layout = dbc.Container(
    [
        html.H3(
            "Weekly Timesheet DataSpace (Satts Cyber Tech Pvt. Ltd.)",
            className="my-3",
        ),
        dbc.Row(
            [
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(
                            options=[
                                {"label": k, "value": k}
                                for k in sorted(EMPLOYEE_DATA.keys())
                            ],
                            id="emp-dropdown",
                            value=sorted(EMPLOYEE_DATA.keys())[0],
                        ),
                    ],
                    width=3,
                ),
                ...
                dbc.Col(
                    [
                        html.Label("Week Start (Monday)"),
                        dcc.DatePickerSingle(
                            id="week-picker", date=current_week_start.isoformat()
                        ),
                    ],
                    width=3,
                ),
                dbc.Col(
                    [
                        ...
                        dbc.Button(
                            "✅ Submit Week", id="submit-btn", color="success"
                        ),
                    ],
                    width=3,
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    dbc.Button("➕ Add Task", id="add-task-btn", color="info"),
                    width=2,
                ),
            ],
            className="mb-3",
        ...
        dbc.Row(
            [
                dbc.Col(
                    html.Div("TASK / SUBTASK", style={"fontWeight": "bold"}),
                    width=2,
                ),
                *[
                    dbc.Col(
                    ...
                    for d in [
                        current_week_start + timedelta(days=i) for i in range(7)
                    ]
                ],
                dbc.Col(
                    html.Div("TOTAL", style={"fontWeight": "bold"}), width=1
                ),
        ...
        dbc.Row(
            [
                dbc.Col(
                    html.Div("WEEKLY TOTAL:", style={"fontWeight": "bold"}),
                    width=2,
                ),
                dbc.Col(
                    html.Div(id="weekly-total", style={"fontWeight": "bold"}),
                    width=2,
                ),
            ]
        ),
...
# ------------------ Master data/UI callback ------------------
@app.callback(
    Output({'type': 'hours-display', 'index': ALL}, 'children'),
    ...
    State("ui-store", "data"),
    State("week-picker", "data"),
    State({"type": "notes-input", "index": ALL}, "value"),
    State({"type": "notes-input", "index": ALL}, "id"),
    prevent_initial_call=True,
)

def update_running_time(n, data_store, emp, week_date, ui_store):
 
    running = ui_store.get("running")
    if not running:
        return no_update
 
    row_idx, day_idx = running
    key = f"{emp}::{week_date}"
    rows = data_store.get(key, {}).get("rows", [])
 
    try:
        day = rows[row_idx]["days"][day_idx]
    except:
        return no_update
 
    hrs = day_total_hours(day)
    return format_hours_hhmm(hrs)
...
def handle_all_updates(
    ...
    # Employee or week change: load/setup entry
    if triggered_id in ("emp-dropdown", "week-picker"):
        ...
        task_options = [
            {"label": t, "value": t}
            for t in sorted(DEPARTMENT_TASKS.get(dept_val, {}).keys())
        ]
            ...
            if running and running != [row_idx, day_idx]:
                ...
                if r < len(rows) and d < len(rows[r].get("days", [])):
                    ...
                    if prev_day.get("running_start"):
                        prev_day["sessions"].append(
                            [prev_day["running_start"], now_iso()]
                        )
    ...
    # Save (persist notes)
        ...
        if note_values and note_ids:
            ...
            if rows:
                notes_map = {
                    note_ids[i]["index"]: note_values[i] for i in range(len(note_ids))
                }
    ...
    # Submit week (Excel + optional server POST)
...
def update_subtasks(task_value, dept_value):
    ...
    return [
        {"label": s, "value": s}
        for s in DEPARTMENT_TASKS.get(dept_value, {}).get(task_value, [])
    ]


# ------------------ Render rows & weekly total ------------------
@app.callback(
